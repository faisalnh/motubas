...
> [!IMPORTANT]
> **Before any feature work, always read these files:**
> - **`.clinerules`** → Detailed MVP specs, file-by-file breakdown, ADR index
> - **`skills/*.md`** → Domain-specific implementation guides
> - **`docs/adr/`** → Architecture Decision Records
>
> This file provides quick reference. `.clinerules` has the complete requirements.
...
### User Flow
1. User signs up → FREE tier, 10 AI credits
2. User adds car → Max 1 for FREE (check `subscriptionTier`)
3. User adds service → Creates auto reminders based on service type
4. Service with cost → Invoice photo REQUIRED (except self-service)
...
2. ✅ Check Prisma schema - Run `npx prisma format` and `npx prisma validate`
3. ✅ Test authentication flow - Login, register, OAuth
4. ✅ Test free tier limits - Try adding 2nd car (should fail)
...
This project uses **Agent Skills** for domain-specific knowledge. Skills are stored in `skills/` with `SKILL.md` files that provide focused, reusable instructions.

### Current Skills

| Skill | Purpose |
|-------|---------|
| `skills/prisma/SKILL.md` | Database query patterns and Prisma best practices |
| `skills/service-records/SKILL.md` | Service record CRUD with invoice handling |
| `skills/service-records/PHOTO-UPLOAD.md` | Image upload with compression |
| `skills/ai-assistant/SKILL.md` | Gemini Flash integration |
| `skills/ai-assistant/CHAT-COMPONENT.md` | Om Motu chat UI patterns |

### When to Create a SKILL.md
- Domain-specific workflows (e.g., car service processing)
- Complex multi-step processes (e.g., invoice upload)
- Reusable patterns (e.g., Prisma queries)
- Integration guides (e.g., AI chat)

---

## Architecture Decision Records (ADRs)

Technical decisions are documented in `docs/adr/`. See `.clinerules` for the complete ADR index.

**Key Decisions:**
- ADR-0002: Next.js 15 with App Router
- ADR-0003: Prisma 7 with PostgreSQL
- ADR-0004: Google Gemini Flash for AI
- ADR-0005: Auth.js v5 for authentication
- ADR-0006: Development best practices (TDD, Conventional Commits, Gitflow)

**Create an ADR when deciding:** technology choices, architectural patterns, API design, security approaches, third-party integrations.
